  ...
  try {
    ...
    if (process.env.SMTP_USER && process.env.SMTP_PASS) {
      ...
      await transporter.sendMail({
        from: `"Skin Solution Booking" <${process.env.SMTP_USER}>`,
        ...
        html: `
          <div style="font-family:'Georgia',serif;max-width:580px;margin:0 auto;padding:32px;background:#0f1f5c;color:#fdfbf7;border-radius:16px">
            <div style="text-align:center;margin-bottom:24px">
              <div style="background:linear-gradient(135deg,#0ea5a0,#0c6b68);display:inline-block;padding:10px 20px;border-radius:8px;font-size:18px;font-weight:bold;letter-spacing:2px">Skin Solution</div>
            </div>
      ...
      // Confirm to patient
      await transporter.sendMail({
        from: `"Dr. Roy — Skin Solution" <${process.env.SMTP_USER}>`,
        to: email,
        ...
        html: `
          <div style="font-family:'Georgia',serif;max-width:560px;margin:0 auto;padding:32px;background:#fdfbf7;border-radius:16px;border:1px solid #e0e8f0">
            <div style="text-align:center;margin-bottom:24px">
              <div style="background:linear-gradient(135deg,#0ea5a0,#0c6b68);display:inline-block;padding:10px 20px;border-radius:8px;color:white;font-size:16px;font-weight:bold;letter-spacing:2px">Skin Solution</div>
            </div>
            <h2 style="color:#0f1f5c;font-size:22px;margin-bottom:16px">Appointment Confirmed</h2>
            <p style="color:#374151;font-size:14px;margin-bottom:24px">Dear ${name},<br><br>Thank you for booking with Dr. Roy at Skin Solution. Your appointment details are below.</p>
