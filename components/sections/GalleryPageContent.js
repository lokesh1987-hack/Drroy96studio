...
export default function GalleryPageContent() {
  ...
  const filtered = cat === 'All' ? siteConfig.galleryPosts : siteConfig.galleryPosts.filter(p => p.category === cat)
    ...
    <div className="pt-16">
      ...
      <div className="section-pad text-center" style={{ background:'linear-gradient(135deg, #070e2d, #0f1f5c)' }}>
        <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }}>
          <span className="font-heading text-xs tracking-widest uppercase mb-4 block" style={{ color:'#5de8d8' }}>— Visual Diary</span>
      ...
      <section className="section-pad bg-ivory">
        <div className="wrap">
          ...
          <div className="flex flex-wrap gap-2 mb-10 justify-center">
            {CATS.map(c => (
              <motion.button key={c} whileTap={{ scale:0.96 }} onClick={() => setCat(c)}
                className="px-5 py-2 rounded-full font-heading text-sm font-medium transition-all"
                style={{
                  background: cat===c ? 'linear-gradient(135deg, #0ea5a0, #0c6b68)' : 'white',
                  color: cat===c ? 'white' : '#374151',
                  border: cat===c ? 'none' : '1px solid rgba(14,165,160,0.2)',
                  boxShadow: cat===c ? '0 4px 16px rgba(14,165,160,0.3)' : '0 2px 8px rgba(0,0,0,0.06)',
                }}>
                {c}
              </motion.button>
            ))}
          ...
          <div className="columns-2 md:columns-3 gap-4">
            <AnimatePresence>
              {filtered.map((post, i) => (
                ...
                >
                  <div className={`flex items-center justify-center ${i%3===0 ? 'h-64' : 'h-48'} relative`}>
                    ...
                    <div className="absolute inset-0 bg-navy-900/0 group-hover:bg-navy-900/50 transition-all duration-300 flex items-center justify-center">
                      <ExternalLink className="w-7 h-7 text-white opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-navy-900/85 to-transparent">
                    <div className="flex items-center justify-between">
                      <p className="font-heading text-xs text-white/80 font-medium truncate pr-2">{post.caption}</p>
                  ...
                  <div className="absolute top-2 left-2 px-2 py-0.5 rounded-full font-heading text-xs font-semibold"
                    style={{ background:'rgba(14,165,160,0.25)', color:'#5de8d8', border:'1px solid rgba(14,165,160,0.3)' }}>
                    {post.category}
                  </div>
                </motion.div>
              ))}
          ...
          <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
            ...
            <div className="flex gap-3 shrink-0">
              <a href={siteConfig.instagram} target="_blank" rel="noopener noreferrer" className="btn-primary text-sm py-3">
      ...
      <AnimatePresence>
        {sel && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            ...
            style={{ background:'rgba(7,14,45,0.85)', backdropFilter:'blur(10px)' }}>
            <motion.div
            ...
            >
              <div className="h-64 flex items-center justify-center text-8xl relative"
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-heading font-semibold px-2.5 py-0.5 rounded-full"
                    style={{ background:'rgba(14,165,160,0.1)', color:'#0ea5a0' }}>{sel.category}</span>
                  <span className="flex items-center gap-1 font-heading text-xs text-ink/40">
                    <Heart className="w-3 h-3 fill-red-400 text-red-400" />{sel.likes} likes
                  </span>
                </div>
                <h3 className="font-display text-lg font-semibold text-navy mb-3">{sel.caption}</h3>
                <a href={siteConfig.instagram} target="_blank" rel="noopener noreferrer"
